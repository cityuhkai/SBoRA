...
import bitsandbytes.functional as qF
...
def mark_only_lora_as_trainable(model: nn.Module, bias: str = "none") -> None:
    for n, p in model.named_parameters():
        ...
        if ("lora_" not in n) and ("magnitude" not in n):
            p.requires_grad = False
        #***************modified****************** 
    if bias == "none":
        return
...
#**********modified 1************
import random
class SB_lora_linear(nn.Module):
    def __init__(self, infeature, outfeature, bias, index = []):
        super(SB_lora_linear, self).__init__()
        self.weight = nn.Parameter(torch.zeros(outfeature, infeature), requires_grad=False)
        if not bias:
            self.register_parameter('bias', None)
        else:
            self.bias = nn.Parameter(torch.zeros(outfeature), requires_grad=False)

        self.index = index
        if len(index) == 0:
            min_rank = min(infeature, outfeature)
            max_rank = max(infeature, outfeature)
            self.index = random.sample(range(max_rank), min_rank)
        
        if self.weight.shape[1] > self.weight.shape[0]:
            for num, I in enumerate(self.index):
                self.weight[num][I] = 1
        else:
            for num, I in enumerate(self.index):
                self.weight[I][num] = 1
...
#*****************modified 2************
    ...
    def update_layer(self, adapter_name, r, lora_alpha, lora_dropout, init_lora_weights):
        ...
        # Actual trainable parameters
        if r > 0:
            # add SB_lora / lora / lora_FA based on the adapter_name:
            if adapter_name == 'default':
                self.lora_A.update(nn.ModuleDict({adapter_name: nn.Linear(self.in_features, r, bias=False)}))
                self.lora_B.update(nn.ModuleDict({adapter_name: nn.Linear(r, self.out_features, bias=False)}))
            elif adapter_name == 'SB_FA_mul':
                self.lora_A.update(nn.ModuleDict({adapter_name: SB_lora_linear(self.in_features, r, bias = False)}))
                self.lora_B.update(nn.ModuleDict({adapter_name: nn.Linear(r, self.out_features, bias=False)}))
            elif adapter_name == 'SB_FB_mul':
                self.lora_A.update(nn.ModuleDict({adapter_name: nn.Linear(self.in_features, r, bias=False)}))
                self.lora_B.update(nn.ModuleDict({adapter_name: SB_lora_linear(r, self.out_features, bias = False)}))
            elif adapter_name == 'SB_FA_add' or adapter_name == 'SB_FA_sampleMul':
                SB_index = torch.tensor(random.sample(range(self.in_features), r))
                self.lora_A = nn.ParameterDict({})
                self.lora_A.update(nn.ParameterDict({adapter_name: nn.Parameter(SB_index, requires_grad=False)}))
                self.lora_B.update(nn.ModuleDict({adapter_name: nn.Linear(r, self.out_features, bias=False)}))
            elif adapter_name == 'SB_FB_add' or adapter_name == 'SB_FB_sampleMul':
                SB_index = torch.tensor(random.sample(range(self.out_features), r))
                self.lora_B = nn.ParameterDict({})
                self.lora_B.update(nn.ParameterDict({adapter_name: nn.Parameter(SB_index, requires_grad=False)}))
                self.lora_A.update(nn.ModuleDict({adapter_name: nn.Linear(self.in_features, r, bias=False)}))
            elif adapter_name == 'lora_FA' or adapter_name == 'lora_FA_uniform':
                self.lora_A.update(nn.ModuleDict({adapter_name: nn.Linear(self.in_features, r, bias=False)}))
                ...
                self.lora_A.update(nn.ModuleDict({adapter_name: nn.Linear(self.in_features, r, bias=False)}))
                self.lora_B.update(nn.ModuleDict({adapter_name: nn.Linear(r, self.out_features, bias=False)}))
                self.magnitude.update(nn.ModuleDict({adapter_name: nn.Linear(1, self.out_features, bias=False)}))
                self.magnitude[adapter_name].weight.data.copy_( (torch.linalg.norm(self.weight.detach(), dim=1).detach()).view(-1,1) )
...
#**************modified 3*******************
    ...
    def reset_lora_parameters(self, adapter_name):
        ...
        elif adapter_name in ['SB_FA_mul', 'SB_FA_add', 'SB_FA_sampleMul']:
            if adapter_name in self.lora_B.keys():
                nn.init.zeros_(self.lora_B[adapter_name].weight)
        elif adapter_name in ['SB_FB_mul', 'SB_FB_add', 'SB_FB_sampleMul']:
            if adapter_name in self.lora_A.keys():
                nn.init.zeros_(self.lora_A[adapter_name].weight)
        elif adapter_name == 'lora_FA':
            if adapter_name in self.lora_A.keys():
                nn.init.normal_(self.lora_A[adapter_name].weight, mean=0, std=1)
                nn.init.zeros_(self.lora_B[adapter_name].weight)

        ## test on different lora_FA initialization
        elif adapter_name == 'lora_FA_uniform':
            if adapter_name in self.lora_A.keys():
                nn.init.kaiming_uniform_(self.lora_A[adapter_name].weight, a=math.sqrt(5))
                nn.init.zeros_(self.lora_B[adapter_name].weight)       
...
#**********************modified 4*****************
            ...
            #     return result
            def forward(self, x: torch.Tensor):
                if self.active_adapter in ['default', 'SB_FA_mul', 'SB_FB_mul', 'lora_FA', 'lora_FA_uniform']:
                    result = super().forward(x)
                    if self.disable_adapters or self.active_adapter not in self.lora_A.keys():
                        return result
                    elif self.r[self.active_adapter] > 0:
                ...
                elif self.active_adapter in ['SB_FA_add', 'SB_FB_add']:
                    # dF means functional in bitsandbytes
                    dequanWeight = qF.dequantize_4bit(self.weight, self.weight.quant_state).to(x.dtype)

                    bias = None if self.bias is None else self.bias.to(self.compute_dtype)

                    if self.active_adapter == 'SB_FA_add':
                        index = self.lora_A[self.active_adapter].detach()
                        mergedWeight = dequanWeight.index_add_(1, index, self.lora_B[self.active_adapter].weight*self.scaling[self.active_adapter])
                            
                    else: ##'SB_FB_add'
                        index = self.lora_B[self.active_adapter].detach()
                        mergedWeight = dequanWeight.index_add_(0, index, self.lora_A[self.active_adapter].weight*self.scaling[self.active_adapter])

                    result = torch.nn.functional.linear(x, mergedWeight, bias)

                elif self.active_adapter in ['SB_FA_sampleMul', 'SB_FB_sampleMul']:
                    result = super().forward(x)
                    if self.disable_adapters or self.active_adapter not in self.lora_A.keys():
                        return result
                    elif self.r[self.active_adapter] > 0:
                        result = result.clone()
                        if self.active_adapter == 'SB_FA_sampleMul':
                            #detach() will not consume additional memmory usage
                            expected_dtype = result.dtype
                            x = x.to(self.lora_B[self.active_adapter].weight.dtype)
                            index = self.lora_A[self.active_adapter].detach()
                            output = (
                                self.lora_B[self.active_adapter](
                                (self.lora_dropout[self.active_adapter](x))[:, :, index]
                            ).to(expected_dtype)
                            * self.scaling[self.active_adapter]
                            )
                        ...
                        if self.active_adapter == 'SB_FB_sampleMul':
                            expected_dtype = result.dtype
                            x = x.to(self.lora_A[self.active_adapter].weight.dtype)
                            output = (
                                self.lora_A[self.active_adapter](self.lora_dropout[self.active_adapter](x)
                            ...
                            * self.scaling[self.active_adapter]
                            ...
                            result = result.index_add_(2, self.lora_B[self.active_adapter], output)
                            # result[:, :, self.lora_B[self.active_adapter]] += output
                    
                elif self.active_adapter == 'dora':
                    result = super().forward(x)
                    if self.disable_adapters or self.active_adapter not in self.lora_A.keys():
                        return result
                    elif self.r[self.active_adapter] > 0:
                        ...
                        dequanWeight = qF.dequantize_4bit(self.weight, self.weight.quant_state).to(x.dtype)
                        column_norm = (dequanWeight + self.lora_B[self.active_adapter].weight @ self.lora_A[self.active_adapter].weight).norm(p=2, dim=1).detach()

                        result += output
                        result = result / column_norm.view(1,1,-1)

                        if torch.all(self.lora_B[self.active_adapter].weight @ self.lora_A[self.active_adapter].weight == 0):
                            self.magnitude[self.active_adapter].weight.data.copy_( (dequanWeight.detach()).norm(p=2, dim=1).unsqueeze(1) )

                        result = result * (self.magnitude[self.active_adapter].weight.view(1,1,-1))
