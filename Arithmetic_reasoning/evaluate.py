...
def main(
    ...
    #********************modified*****************************
    if args.adapter == "LoRA" or args.adapter == "DoRA" or args.adapter == "SBoRA_FA" or args.adapter == "SBoRA_FB" or args.adapter == "LoRA_FA":
        print("Merge LoRA/DoRA/SBoRA/LoRAfa weights into the original weights")
...
def load_data(args) -> list:
    ...
    """
    file_path = f'./Datasets/{args.dataset}/test.json'
...
def parse_args():
    ...
    parser.add_argument('--dataset', choices=['AddSub', 'MultiArith', 'SingleEq', 'gsm8k', 'AQuA', 'SVAMP'],
                        required=True)
    parser.add_argument('--model', choices=['LLaMA-7B', 'BLOOM-7B', 'GPT-j-6B', "LLaMA3-8B", "LLaMA2-7B"], required=True)
    #******************modified*****************
    # parser.add_argument('--adapter', choices=['LoRA', 'AdapterP', 'AdapterH', 'Parallel', 'Prefix'],
    #                     required=True)
    parser.add_argument('--adapter', choices=['LoRA', 'AdapterP', 'AdapterH', 'Parallel', 'Prefix', 'DoRA', 'SBoRA_FA', 'SBoRA_FB', 'LoRA_FA'],
                        required=True)
    #******************modified*****************
...
def load_model(args) -> tuple:
    ...
    if "LLaMA" in args.model: #*************modified: add grant token****************
        if "Llama-3" in base_model:
            tokenizer = AutoTokenizer.from_pretrained(base_model, token = args.hftoken)
        else:
            tokenizer = LlamaTokenizer.from_pretrained(base_model, token = args.hftoken)
    else:
        tokenizer = AutoTokenizer.from_pretrained(base_model)
    ...
    if device == "cuda":
        ...
        model = AutoModelForCausalLM.from_pretrained(
            base_model, device_map={"": device}, low_cpu_mem_usage=True, token = args.hftoken,
        )
        model = PeftModel.from_pretrained(
            model,
            lora_weights,
