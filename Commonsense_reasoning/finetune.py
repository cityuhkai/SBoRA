...
def train(
    ...
    if model.config.model_type == "llama":
        ...
        if "Llama-3" in base_model:
            ...
            tokenizer = AutoTokenizer.from_pretrained(base_model, token = hgtoken)
        else:
            tokenizer = LlamaTokenizer.from_pretrained(base_model, token = hgtoken)
    else:
        tokenizer = AutoTokenizer.from_pretrained(base_model, trust_remote_code=True, token = hgtoken)
    ...
    #**********modified*****************
        ...
        config = LorafaConfig(
            ...
            bias='none',
            task_type="CAUSAL_LM",
            init="uniform" # or choose "normal"
        )
    elif adapter_name == "sbora_fa" or adapter_name == "sbora_fb":
        print("SBoRA init")
        config = SboraConfig(
            r = lora_r,
            lora_alpha = lora_alpha,
            ...
            bias = "none",
            task_type = "CAUSAL_LM",
            adapter_name = adapter_name # or choose "sbora_fb"
        )
    #**********modified**********************
