...
def load_data(args) -> list:
    ...
    """
    # ****************modified: dataset file path**************
    file_path = f'./Datasets/dataset/{args.dataset}/test.json'
    # *********************************************************
...
def parse_args():
    ...
    #                     required=True)
    parser.add_argument('--adapter', choices=['LoRA', 'AdapterP', 'AdapterH', 'Parallel', 'DoRA', 'SBoRA_FA', 'SBoRA_FB', 'LoRA_FA'],
                        required=True)
    #*****************modified*********************
...
def load_model(args) -> tuple:
    ...
    if "LLaMA" in args.model: #*************modified: add grant token****************
        if "Llama-3" in base_model:
            tokenizer = AutoTokenizer.from_pretrained(base_model, token = args.hgtoken)
        else:
            tokenizer = LlamaTokenizer.from_pretrained(base_model, token = args.hgtoken)
    else:
    ...
    if device == "cuda":
        ...
        model = AutoModelForCausalLM.from_pretrained(
            base_model, device_map={"": device}, low_cpu_mem_usage=True, token = args.hgtoken,
        )
        model = PeftModel.from_pretrained(
            model,
            lora_weights,
